...
class Settings:
    ...
    end_time: str = "17:25"
    telegram_scenarios: set[str] = field(default_factory=lambda: {"ENTRADA", "QUASE"})
    ...
    def load(cls, env_path: Path | None = None) -> "Settings":
        ...
        send_for = {item.strip().upper() for item in os.environ.get("ENVIAR_TELEGRAM_PARA", "ENTRADA,QUASE").split(",") if item.strip()}
        settings = cls(
