...
class MonitorService:
    def __init__(self, settings: Settings, *, log: Callable[[str], None] = print) -> None:
        self.settings = settings
        self.log = log
        self.capture = ScreenCapture(settings.capture_region)
        self.analyzer = AnthropicVisionAnalyzer(settings.anthropic_api_key, settings.model)
    ...
    def process_once(self) -> AnalysisResult:
        ...
        risk = calculate_trade_risk(analysis, max_contracts=self.settings.max_contracts, max_risk_brl=self.settings.max_risk_brl)
        for outcome in self.storage.evaluate_open_signals(analysis.current_price):
            emoji = "✅" if "GANHOU" in outcome.result else "❌"
            self.telegram.send_text(f"{emoji} Resultado do sinal de {outcome.signal.timestamp}: <b>{outcome.result}</b>\n{outcome.signal.signal.value} | Entrada: {outcome.signal.entry} | Preco observado: {outcome.observed_price}")
            result_analysis = AnalysisResult(scenario_type=ScenarioType.RESULT, signal=outcome.signal.signal, confidence=Confidence.MEDIUM, current_price=outcome.observed_price, suggested_entry=outcome.signal.entry, suggested_stop=outcome.signal.stop, suggested_target1=outcome.signal.target1, suggested_target2=outcome.signal.target2, rationale=f"Resultado aproximado, observado nos ciclos de 5 minutos, referente ao sinal de {outcome.signal.timestamp}.")
            self.storage.append_analysis_log(result_analysis, "-", outcome.result)
        ...
        self.log(f"  -> {analysis.scenario_type.value}: {analysis.rationale[:120]}")
        if analysis.scenario_type.value in self.settings.telegram_scenarios:
            self.telegram.send_photo(study_image, telegram_caption(analysis, risk))
        if risk and risk.recommended_contracts == 0:
            self.telegram.send_text("⛔ Setup tecnico detectado, mas o risco financeiro configurado nao permite nem 1 contrato. Entrada bloqueada.")
        return analysis

    def run_forever(self, stop_event: threading.Event | None = None) -> None:
        stop_event = stop_event or threading.Event()
        self.settings.data_dir.mkdir(parents=True, exist_ok=True)
        self.telegram.send_text("🚀 WIN Monitor iniciado em modo estudo. Nao executarei ordens; enviarei ENTRADA e QUASE para revisao.")
        self.log("WIN Monitor iniciado em modo estudo.")
        while not stop_event.is_set():
            now = datetime.now()
            if not within_trading_hours(now, self.settings.start_time, self.settings.end_time):
