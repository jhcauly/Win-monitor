...
class StudyImageComposer:
    def compose(
        ...
        y = 12
        title = (
            f"{analysis.scenario_type.value} - {analysis.signal.value} "
            f"({analysis.confidence.value}) | {datetime.now().strftime('%d/%m %H:%M')}"
        )
        draw.text(
            (16, y),
            title,
            font=title_font,
            fill=colors.get(analysis.scenario_type, (200, 200, 200)),
        )
        y += 38

        lines = [
            f"Vies M60: {analysis.bias_m60}",
            f"Estrutura M15: {analysis.structure_m15}",
            f"Zona Fibo M5: {analysis.fib_zone_m5}",
        ]
        if analysis.scenario_type is ScenarioType.ENTRY:
            lines.append(
                f"Entrada: {analysis.suggested_entry} | "
                f"Stop: {analysis.suggested_stop} | "
                f"Alvo1: {analysis.suggested_target1} | "
                f"Alvo2: {analysis.suggested_target2}"
            )
        elif analysis.scenario_type is ScenarioType.ALMOST:
            code = analysis.missing_confirmation_code or "NAO_INFORMADO"
            reason = analysis.missing_confirmation or "-"
            lines.append(f"Faltou [{code}]: {reason}")

        if risk:
            lines.append(
                f"Risco: {risk.risk_points:.0f} pts | "
                f"1 contrato: R${risk.risk_per_contract_brl:.2f} | "
                f"5 contratos: R${risk.risk_five_contracts_brl:.2f}"
            )
            lines.append(
                f"Quantidade recomendada: {risk.recommended_contracts} | "
                f"Risco recomendado: R${risk.recommended_risk_brl:.2f}"
            )
    ...
    def _fonts() -> tuple[ImageFont.ImageFont, ImageFont.ImageFont]:
        ...
        for title_name, text_name in candidates:
            try:
                return (
                    ImageFont.truetype(title_name, 22),
                    ImageFont.truetype(text_name, 16),
                )
            except OSError:
                continue
